// Traduções amigáveis (pt-BR) para mensagens do sistema de autenticação.
...
export const PASSWORD_RULES: PasswordRule[] = [
  { id: "len", label: "No mínimo 8 caracteres", test: (v) => v.length >= 8 },
  { id: "upper", label: "Pelo menos uma letra maiúscula", test: (v) => /[A-ZÀ-Ý]/.test(v) },
  { id: "lower", label: "Pelo menos uma letra minúscula", test: (v) => /[a-zà-ÿ]/.test(v) },
...
export function translateAuthError(err: unknown, fallback = "Não foi possível continuar. Tente novamente."): string {
  const raw =
    typeof err === "string"
      ? err
  ...
  // Se a mensagem tem caracteres típicos de português, mostra como está.
