// Classificação de intenção das mensagens recebidas (server-only).
// Camada 1: heurística (regex) — instantânea e determinística.
// Camada 2: IA (Lovable AI) — pega frases indiretas que a regex não cobre.
// Falha sempre em silêncio para nunca travar o fluxo do WhatsApp.

export type LeadIntent = "opt_out" | "handoff_interno" | "outro";

export async function classifyLeadIntent(text: string): Promise<{ intent: LeadIntent; source: "heuristic" | "ai" | "default"; reason?: string }> {
  const t = (text ?? "").trim();
  if (!t) return { intent: "outro", source: "default" };

  const { isExplicitOptOut, looksLikeInternalHandoff } = await import("./optout");
  if (isExplicitOptOut(t)) return { intent: "opt_out", source: "heuristic", reason: "Padrão explícito de recusa/remoção" };
  if (looksLikeInternalHandoff(t)) return { intent: "handoff_interno", source: "heuristic", reason: "Encaminhamento interno detectado" };

  const key = process.env.LOVABLE_API_KEY;
  if (!key) return { intent: "outro", source: "default" };
  try {
...
Na dúvida entre OPT_OUT e OUTRO, escolha OPT_OUT somente se houver recusa clara do assunto/produto.`,
    ...
    if (clean.startsWith("OPT_OUT")) return { intent: "opt_out", source: "ai", reason: "Recusa identificada pela IA" };
    if (clean.startsWith("HANDOFF")) return { intent: "handoff_interno", source: "ai", reason: "Encaminhamento interno identificado pela IA" };
    return { intent: "outro", source: "ai" };
  } catch {
    return { intent: "outro", source: "default" };
  }
}
