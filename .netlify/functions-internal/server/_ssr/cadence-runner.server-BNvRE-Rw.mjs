import { sendAndLog } from "./messaging.server-Czbp4TxB.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/cadence-runner.server-BNvRE-Rw.js
async function loadAdmin(workspaceId) {
	const { wsDb } = await import("./workspace-scope.server-BnuHkW86.mjs").then((n) => n.t).then((n) => n.t);
	return await wsDb(workspaceId);
}
/** Teto de segurança por execução para a progressão automática (Dia 2+). */
var FOLLOWUP_CAP = 2e3;
function firstName(name) {
	return (name ?? "").trim().split(/\s+/)[0] ?? "";
}
function renderScript(tpl, ctx) {
...
async function endCadence(admin, contactId, reason) {
	const in60 = new Date(Date.now() + 5184e6).toISOString();
...
async function hasReplied(admin, contactId) {
	const { data } = await admin.from("activities").select("id").eq("contact_id", contactId).eq("kind", "whatsapp_in").limit(1);
...
*/
async function runCadenceBatch(workspaceId, slot, batchSize) {
	...
	const { data: steps } = await admin.from("cadence_steps").select("day, script, active").eq("active", true).order("day", { ascending: true });
	const stepList = steps ?? [];
	if (stepList.length === 0) return result;
	const maxDay = stepList[stepList.length - 1].day;
	const scriptByDay = new Map(stepList.map((s) => [s.day, s.script]));
	const today = /* @__PURE__ */ new Date();
	today.setHours(0, 0, 0, 0);
	const todayIso = today.toISOString();
	const select = "id, name, whatsapp, phone, cadence_day, last_contact_at, funnel_stage";
	const eligible = (q) => q.eq("cadence_active", true).eq("do_not_contact", false).eq("is_bot", false).eq("ai_paused", false).lt("cadence_day", maxDay).or(`last_contact_at.is.null,last_contact_at.lt.${todayIso}`);
	const { data: followUpsRaw } = await eligible(admin.from("contacts").select(select).gte("cadence_day", 1)).order("cadence_day", { ascending: false }).order("last_contact_at", {
		ascending: true,
		nullsFirst: true
	}).limit(FOLLOWUP_CAP);
	const followUps = followUpsRaw ?? [];
	const { data: newRaw } = await eligible(admin.from("contacts").select(select).eq("cadence_day", 0).eq("funnel_stage", "novo_lead")).order("last_contact_at", {
		ascending: true,
		nullsFirst: true
	}).limit(Math.max(0, batchSize));
	const newLeads = newRaw ?? [];
	...
	for (const c of list) {
		...
		const { data: fresh } = await admin.from("contacts").select("cadence_active, cadence_day, do_not_contact, is_bot, ai_paused").eq("id", c.id).maybeSingle();
		const f = fresh ?? {};
		...
		const { data: sentToday } = await admin.from("activities").select("id").eq("contact_id", c.id).eq("kind", "whatsapp_out").gte("created_at", todayIso).limit(1);
...
*/
async function autoReplyToInbound(params) {
	const admin = await loadAdmin(params.workspaceId);
	const { data: settingsRow } = await admin.from("cadence_settings").select("auto_reply_enabled").maybeSingle();
	if (!(settingsRow ?? {}).auto_reply_enabled) {
		console.log("[eva auto-reply] desativado nas configurações");
		return "skipped:auto_reply_disabled";
	}
	const { data: contactRow } = await admin.from("contacts").select("do_not_contact, status, is_bot, ai_paused").eq("id", params.contactId).maybeSingle();
	...
	const { data: stepRow } = await admin.from("cadence_steps").select("script, ai_instructions").eq("day", day).maybeSingle();
	const step = stepRow ?? {};
	let instructions = (step.ai_instructions ?? "").trim();
	if (!instructions) {
		const { data: anyStep } = await admin.from("cadence_steps").select("ai_instructions").neq("ai_instructions", "").order("day", { ascending: true }).limit(1).maybeSingle();
		instructions = (anyStep?.ai_instructions ?? "").trim();
	}
	if (!instructions) instructions = "Se o cliente demonstrar interesse, ofereça agendar uma reunião de 15 minutos. Se pedir para não receber mais, encerre educadamente.";
...
Responda APENAS com o texto da mensagem que deve ser enviada ao cliente ${params.contactName}. Nada de "aqui está a resposta:" ou aspas.`;
	...
	try {
		const { text } = await generateText({
			...
			messages: [{
				role: "user",
				content: params.incomingText || "(cliente respondeu)"
			}]
		});
		reply = (text ?? "").trim();
	} catch (err) {
	...
	const { isExplicitOptOut, looksLikeFarewell } = await import("./optout-8SOkuOwY.mjs");
	...
	if (looksLikeFarewell(reply) && !farewellAllowed) {
		...
		try {
			...
			else reply = params.signal === "handoff_interno" ? "Perfeito, obrigado por levar internamente! Quer que eu te envie um resumo de 1 página para facilitar o repasse ao time? E qual prazo você imagina para o retorno — posso te chamar na próxima semana?" : "Perfeito! Me confirma o melhor dia e horário para você que eu já reservo na agenda.";
		} catch {
			reply = params.signal === "handoff_interno" ? "Perfeito, obrigado por levar internamente! Quer que eu te envie um resumo curto para facilitar o repasse ao time? Consigo te chamar na próxima semana para saber como ficou?" : "Perfeito! Me confirma o melhor dia e horário para você que eu já reservo na agenda.";
		}
	}
...
//#endregion
export { autoReplyToInbound, runCadenceBatch };
